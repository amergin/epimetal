...
These instructions are provided in case you want to run your own instance of EpiMetal. The software is encapsulated in [Docker](http://docker.com) containers, making the software relatively easy to deploy on different host systems.

## Technical overview
There are four Docker containers that are set up in order to run EpiMetal. 
...
`$ git clone https://github.com/amergin/epimetal.git`
...
In the `epimetal` directory, issue the following command:
...
Next, open the file `Dockerfile` in the epimetal root directory using a text editor. Look for the line 

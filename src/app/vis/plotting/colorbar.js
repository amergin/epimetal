// see http://bl.ocks.org/chrisbrich/4209888#colorbar.js
function colorBar() {
    var orient = "right",
    ...
    function component(selection) {
        ...
        //Enters
        var bar = container.selectAll("path.c").data(d3.range(quads.length), function(d) {
            return d;
        }),
        ...
        a = container.selectAll("g.axis").data(function(d) {
            return (aScale) ? [1] : [];
            }), //axis container
        ...
        aTransform = function(selection, lw) {
            selection.attr("transform", "translate(" + (((orient == "right") || (orient == "left")) ? -lw / 2 : 0) + "," +
        ...
        function lineJoin(p0, p1, p2, p3, width) {
            ...
            if (p3) { // clip ab and dc using average of u12 and u23
                var u23 = perp(p2, p3),
                gg = [p2[0] + u23[0] + u12[0], p2[1] + u23[1] + u12[1]];
                b = lineIntersect(p2, gg, a, b);
                c = lineIntersect(p2, gg, d, c);
            }
...
component.lineWidth = function(x) {
    if (!arguments.length) {
        return lineWidth;
    }
    lineWidth = x;
...
component.tickFormat = function(x) {
    if (!arguments.length) {
        return tickSizex;
    }
    tickSizex = x;
...
component.precision = function(x) {
    if (!arguments.length) {
        return precision;
    }
    precision = x;

...
.factory('RegressionService', function RegressionService($injector, $q, DatasetFactory, TabService, _) {
  var that = this;
  var service = {};
  ...
  function threadFunctionNumericjs(thData) {
    ...
    var getNormalizedData = function(data) {
      var process = function(array) {
        var ret = [],
          avg = mean(array),
          stDev = stDeviation(array, avg, function(d) {
            return +d;
          });
    ...
    try {
      _.each(thData.data, function(obj) {
        var computation = compute({
            ...
            nans: _.find(global.env.nanIndices, function(d) {
              return d.name == obj.name;
            }).nans,
            target: _.find(global.env.targetData, function(d) {
              return d.name == obj.name;
            }).samples,
            adjustData: _.find(global.env.adjustData, function(d) {
              return d.name == obj.name;
            }).samples
          }),
          result = _.chain(obj)
